...
class SystemModule():
    ...
    def status(self) -> str:
        ...
        """
        for attempt in range(3):  # 3 pokusy celkem
            try:
    ...
    def ready_to_start_analysis(self, modules=["CE1", "DAD1"], timeout=None, verbose=True):
        """
        Kontroluje, zda jsou všechny moduly připravené (ve stavu Idle) pro spuštění analýzy.
        
        Args:
            modules: Seznam modulů ke kontrole (default: ["CE1", "DAD1"])
            timeout: Maximální doba čekání v sekundách. None = čekat do nekonečna
            verbose: Zobrazovat průběžný status (default: True)
        
        Returns:
            True když jsou všechny moduly připravené
        
        Raises:
            TimeoutError: Když moduly nejsou připravené v daném časovém limitu
        """
        # Vytvoření příkazu s oddělovačem "|" mezi hodnotami
        command_parts = []
        ...
        while True:
            # Poslání příkazu s retry logikou
            response = None
            for attempt in range(3):  # 3 pokusy celkem
                try:
                    response = self.comm.send(command, 10)
                    break  # Úspěšně jsme dostali odpověď
                except TimeoutError:
                    if attempt == 2:  # Poslední pokus
                        raise
            
            
            # Parsování odpovědi s oddělovačem "|"
            ...
            if timeout is not None:
                ...
                if elapsed >= timeout:
                    if verbose:
                        print()  # Nový řádek před chybou
                    raise TimeoutError(f"Přístroj nebyl připravený na analýzu v časovém limitu {timeout} sekund. "
                                    f"Stavy modulů: {', '.join(module_states)}")
            
            # Čekání před dalším pokusem
            time.sleep(1)
